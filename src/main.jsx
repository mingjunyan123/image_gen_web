...
function validateGptDimensions(widthInput, heightInput) {
  ...
  if (!widthText || !heightText) {
    return { valid: false, code: "missing", error: "宽高必须填写完整" };
  }
  if (!/^\d+$/.test(widthText) || !/^\d+$/.test(heightText)) {
    return { valid: false, code: "integer", error: "宽高必须是正整数" };
  }
  ...
  if (width % 16 !== 0 || height % 16 !== 0) {
    return { valid: false, code: "multipleOf16", error: "宽高必须能被 16 整除" };
  }

  const ratio = width / height;
  if (ratio < 1 / 3 || ratio > 3) {
    return { valid: false, code: "ratio", error: "宽高比必须在 1:3 到 3:1 之间" };
  }

  const longSide = Math.max(width, height);
  const shortSide = Math.min(width, height);
  if (longSide > GPT_MAX_LONG_SIDE || shortSide > GPT_MAX_SHORT_SIDE) {
    return { valid: false, code: "maxSize", error: "最大支持 3840x2160，竖图方向为 2160x3840" };
  }
...
function validateGptSizeString(size) {
  ...
  if (!match) return { valid: false, code: "missing", error: "宽高必须填写完整" };
  return validateGptDimensions(match[1], match[2]);
}

function makeGptRoundOptions(widthInput, heightInput) {
  const width = Number(String(widthInput ?? "").trim());
  const height = Number(String(heightInput ?? "").trim());
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0) return [];
  if (width % 16 === 0 && height % 16 === 0) return [];

  const roundDown = (value) => Math.max(16, Math.floor(value / 16) * 16);
  const roundUp = (value) => Math.ceil(value / 16) * 16;
  const candidates = [
    { label: "向下取整", width: roundDown(width), height: roundDown(height) },
    { label: "向上取整", width: roundUp(width), height: roundUp(height) },
  ];
  const seen = new Set();
  return candidates.filter((candidate) => {
    ...
    seen.add(key);
    return validateGptDimensions(candidate.width, candidate.height).valid;
  });
}
...
function App() {
  ...
  const gptRoundOptions = modelKey === MODEL_KEYS.GPT
    && gptResolutionMode === GPT_RESOLUTION_MODES.CUSTOM
    && customGptValidation.code === "multipleOf16"
      ? makeGptRoundOptions(gptCustomWidth, gptCustomHeight)
      : [];
  ...
  function updateSourceImage(file) {
    ...
    readImageDimensions(nextPreview)
      .then(({ width, height }) => {
        ...
        setGptResolutionMode(GPT_RESOLUTION_MODES.CUSTOM);
      })
      .catch(() => {
        if (sourcePreviewId.current !== previewId) return;
                ...
                <>
                  ...
                  {gptResolutionMode === GPT_RESOLUTION_MODES.PRESET ? (
                    ...
                    <div className="customSizeBlock">
                      ...
                      {gptRoundOptions.length ? (
                        <div className="sizeFixOptions" aria-label="推荐分辨率">
                          {gptRoundOptions.map((option) => (
                            <button
                              key={`${option.width}x${option.height}`}
                              type="button"
                              onClick={() => {
                                setGptCustomWidth(String(option.width));
                                setGptCustomHeight(String(option.height));
                              }}
                            >
